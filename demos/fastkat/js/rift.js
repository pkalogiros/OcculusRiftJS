  ...
  w.rift = {
    ...
    setEulerFromQuaternion : function( order ) {
      ...
      var q = w.rift.quaternion,
          eul = w.rift.eulerAngle,
          angle = w.rift.screenConfig,
          screen = w.rift.screenCoords;
      ...
      // update screenCoords
      screen.x = ((angle.x - Math.min( Math.max( eul.y, -angle.x ), angle.x ) ) / (angle.x * 2)) * 100;
      screen.y = ((angle.y - Math.min( Math.max( eul.x, -angle.y ), angle.y ) ) / (angle.y * 2)) * 100;
